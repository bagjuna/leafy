<template>
  <div>
    ...
    <div class="plant-list">
      <div v-for="userPlant in userPlants" :key="userPlant.userPlantId" class="plant-item"
           @click="openPlantDetailModal(userPlant.plant.plantId)">
        ...
        <div class="plant-details">
          <h5 style="color: green; font-weight: bold; cursor: pointer;" class="plant-name">{{
              userPlant.plantNickname
            }}({{ userPlant.plant ? userPlant.plant.plantName : '알 수 없는 식물' }})</h5>
        ...
        <div class="log-body">
          <v-card-text class="log-title">
            ...
            <h5 style="color: green; font-weight: bold; cursor: pointer;"
                @click="openPlantDetailModal(log.userPlant.plant.plantId)">{{
                log.userPlant.plantNickname
              }}({{ log.userPlant.plant ? log.userPlant.plant.plantName : '알 수 없는 식물' }})</h5> {{ log.note }}
          </v-card-text>
        </div>
    ...
    <PlantDetailModal :deleteButton="false" :isOpen="showPlantDetailModal"
                      @update:isOpen="showPlantDetailModal = $event" :plantId="selectedPlantId"
...
export default {
  ...
  setup() {
    ...
    const fetchRecentLogs = async () => {
      api.get(`/api/plant-logs/recent/user`)
          .then(response => {
